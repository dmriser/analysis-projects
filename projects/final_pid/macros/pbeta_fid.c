{
  ...
  for(int isect=0; isect<NSECT; isect++){
    pbeta[isect] = (TH2F*) histFile->Get(Form("MesonHistograms/h2_p_beta_%d_211_fid_cut", isect));
    printf("loading (%d) \n", isect); 
  }
  ...
  c1->cd();
  tit.DrawLatex(0.1, 0.95, "cut applied: DC fiducial");
  c1->Print(Form("%sp_beta_fid.pdf",imagePath.c_str()));
