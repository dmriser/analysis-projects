{
  ...
  const static int NTYPE = 11; 
  const static int NSECT =  7;


  // Electron Identification Histograms [2][7]                                                                                                                                                                      // [0] - without cuts,  [1] - with cuts                                                                                                                                                                           // [0] - all sectors,   [1-6] - sectors 1-6                                                                                                                                                                  
  string type[NTYPE] = {"allNegatives", "cuts", "Z_VERTEX","CC_FID","CC_PHI","CC_THETA","DC_R1_FID","DC_R3_FID","EC_FID","EC_IN_OUT","EC_SAMPLING"};
  ...
  string cutApplied[NTYPE] = {"None", "All", "Z-Vertex", "CC Fiducial", "CC #phi-Matching", "#theta_{CC} Segment-Matching", "DC Fiducial (R1)", 
			   "DC Fiducial (R3)", "EC Fiducial UVW", "EC Inner-EDep", "Sampling Fraction"};
  ...
  for (int itype = 0; itype < NTYPE; itype++)
	...
	printer = Form("Pass: %.3f ", (double) h1_z_vertex[itype][isect]->GetEntries()/h1_nphe[0][isect]->GetEntries() );
	lab.DrawLatex(xPosFrac, yPosFrac, printer.c_str());
	lab.DrawLatex(xPosCut, yPosCut, Form("Applied Cut: %s", cutApplied[itype].c_str())); 
	...
	printer = Form("Pass: %.3f ", (double) h2_cc_theta[itype][isect]->GetEntries()/h1_nphe[0][isect]->GetEntries() );
	// Draw things 
	...
	printer = Form("Pass: %.3f ", (double) h2_ec_edep[itype][isect]->GetEntries()/h1_nphe[0][isect]->GetEntries() );
	...
	gPad->SetGridy();
	gPad->SetLogz(0);
	printer = Form("Pass: %.3f ", (double) h2_ec_fid[itype][isect]->GetEntries()/h1_nphe[0][isect]->GetEntries() );
	xAxisTitle = "x (cm)"; 
	yAxisTitle = "y (cm)"; 
	xlabel.DrawLatex(0.48, 0.05, xAxisTitle.c_str()); 
	ylabel.DrawLatex(0.05, 0.47, yAxisTitle.c_str()); 
